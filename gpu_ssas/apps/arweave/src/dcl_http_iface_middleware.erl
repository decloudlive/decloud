...
-include_lib("decloud/include/ar_pricing.hrl").
-include_lib("decloud/include/ar_data_sync.hrl").
-include_lib("decloud/include/ar_mine.hrl").
...
handle(Req, Pid) ->
	Peer = ar_http_util:decloud_peer(Req),
	handle(Peer, Req, Pid).
	...
	{200, #{},
		ar_serialize:jsonify(
			...
			||
				...
				P /= ar_http_util:decloud_peer(Req)
			]
		),
	Req};
...
check_if_chunk_data_root_exists(Req) ->
	case cowboy_req:header(<<"decloud-data-root">>, Req, not_set) of
		EncodedDataRoot when byte_size(EncodedDataRoot) == 43 ->
			case cowboy_req:header(<<"decloud-data-size">>, Req, not_set) of
				not_set ->
					continue;
				MaybeNumber ->
...
post_block(request, {Req, Pid}, ReceiveTimestamp) ->
	OrigPeer = ar_http_util:decloud_peer(Req),
...
post_block(read_blockshadow, OrigPeer, {Req, Pid}, ReceiveTimestamp) ->
	HeaderBlockHashKnown =
		case cowboy_req:header(<<"decloud-block-hash">>, Req, not_set) of
			not_set ->
				false;
			EncodedBH ->
...
post_tx_parse_id(check_header, {Req, Pid}) ->
	case cowboy_req:header(<<"decloud-tx-id">>, Req, not_set) of
		not_set ->
