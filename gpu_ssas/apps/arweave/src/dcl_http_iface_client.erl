%%%
%%% @doc Exposes access to an internal decloud client to external nodes on the network.
%%%
...
-include_lib("decloud/include/ar_wallets.hrl").

%% @doc Send a new transaction to an decloud HTTP node.
send_new_tx(Peer, TX) ->
	TXID = TX#tx.id,
	case ets:member(peer_txid, {Peer, TXID}) of
		...
		false ->
			...
			ar_http:req(#{
				...
				peer => Peer,
				path => "/tx",
				headers => p2p_headers() ++ [{<<"decloud-tx-id">>, ar_util:encode(TXID)}],
...
send_new_block(Peer, #block{ height = Height } = NewB, BDS) ->
	...
	ar_http:req(#{
		...
		path => "/block",
		headers =>
			p2p_headers() ++ [{<<"decloud-block-hash">>, ar_util:encode(NewB#block.indep_hash)}],
