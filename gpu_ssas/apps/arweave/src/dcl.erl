%%%
%%% @doc decloud server entrypoint and basic utilities.
%%%
-module(ar).
...
-include_lib("decloud/include/ar_config.hrl").
...
show_help() ->
	io:format("Usage: decloud-server [options]~n"),
	...
	lists:foreach(
		...
		[
			...
			{"disk_space (num)",
				...
				"the decloud data directory (blocks, txs, etc) when "
			...
			{"randomx_bulk_hashing_iterations",
				...
				" to the decloud miner. The faster CPU hashes, the higher this value should be."
			},
			{"disk_cache_size_mb",
...
%% @doc Start an decloud node on this BEAM.
start() ->
...
start(Config) ->
	...
	ok = application:set_env(decloud, config, Config),
	{ok, _} = application:ensure_all_started(decloud, permanent).

start(normal, _Args) ->
	{ok, Config} = application:get_env(decloud, config),
	...
	logger:set_application_level(decloud, info),
...
validate_trusted_peers(Config) ->
	...
	case ValidPeers of
		[] ->
			erlang:halt();
		_ ->
			application:set_env(decloud, config, Config#config{ peers = ValidPeers }),
...
set_mining_address(Config) ->
	...
	application:set_env(decloud, config, Config#config{ mining_addr = MiningAddr }).
...
stop_dependencies() ->
	{ok, [_Kernel, _Stdlib, _SASL, _OSMon | Deps]} = application:get_key(decloud, applications),
...
docs() ->
	Mods =
		lists:filter(
			...
			element(2, file:list_dir("apps/decloud/src"))
		),
	edoc:files(
		["apps/decloud/src/" ++ Mod || Mod <- Mods],
		[
