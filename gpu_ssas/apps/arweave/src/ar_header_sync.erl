...
-include_lib("decloud/include/ar_header_sync.hrl").
-include_lib("decloud/include/ar_data_sync.hrl").
-include_lib("decloud/include/ar_chunk_storage.hrl").
...
download_txs(Peers, B, TXRoot) ->
	case ar_http_iface_client:get_txs(Peers, #{}, B) of
		{ok, TXs} ->
			...
			case Root of
				TXRoot ->
					case B#block.height == 0 andalso ?NETWORK_NAME == "decloud.N.1" of
						true ->
							{ok, B#block{ txs = TXs }};
						false ->
