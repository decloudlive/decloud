...
start(no_block) ->
	[B0] = ar_weave:init([]),
	start(B0, unclaimed, element(2, application:get_env(decloud, config)));
start(B0) ->
	start(B0, unclaimed, element(2, application:get_env(decloud, config))).

start(B0, RewardAddr) ->
	start(B0, RewardAddr, element(2, application:get_env(decloud, config))).
...
start(B0, RewardAddr, Config) ->
	...
	ok = application:set_env(decloud, config, Config#config{
...
disconnect_from_slave() ->
	...
	{ok, Config} = application:get_env(decloud, config),
	application:set_env(decloud, config, Config#config{ peers = [] }),
	{ok, SlaveConfig} = slave_call(application, get_env, [decloud, config]),
	slave_call(application, set_env, [decloud, config, SlaveConfig#config{ peers = [] }]).
