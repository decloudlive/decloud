    ...
    public static class UUID
    {
        ...
        public static string GetDeviceB64UUID(bool showInfoToHash = false)
        {
            ...
            if (macHexOK && systemGuidOK)
            {
                ...
                if (showInfoToHash)
                {
                    Console.WriteLine("DCL/[{cpuSerial}]-[{macHex}]-[{guid}]-[{extraRigSeed}]");
                    Console.WriteLine(infoToHash);
                }
                Logger.Info("DCL.UUID", $"infoToHash='{infoToHash}'");
            ...
            {
                Logger.Error("DCL.UUID", $"Unable to read MAC or GUID read macHexOK='{macHexOK}' systemGuidOK='{systemGuidOK}'");
                Logger.Info("DCL.UUID", $"gotCachedHexUuid cachedHexUuid='{cachedHexUuid}'");
            ...
            else
            {
                ...
                Logger.Info("DCL.UUID", $"Generating hexUuidGenRandom='{hexUuidGenRandom}' ok='{CacheHexUUIDToFile(hexUuidGenRandom)}'");
        ...
        private static string GetInfoToHash(string macHex, string guid)
        {
            ...
            var infoToHash = $"DCL/[{cpuSerial}]-[{macHex}]-[{guid}]-[{extraRigSeed}]";
            return infoToHash;
        }
        ...
        private static (bool ok, string systemGuid) GetMachineGuidOrFallback()
        {
            ...
            {
                Logger.Error("DCL.UUID", $"GetMachineGuid: {e.Message}");
            }

            // fallback deterministic
            try
            {
                const string valueFallback = "MachineGuidDCLGen";
                using (var rkFallback = Registry.CurrentUser.OpenSubKey(@"SOFTWARE\" + APP_GUID.GUID, true))
                {
                    ...
                    if (fallbackUUIDValue == null)
                    {
                        ...
                        catch (Exception e)
                        {
                            ...
                            Logger.Error("DCL.UUID", $"Fallback SetValue: {e.Message}");
                            return (false, "");
                        }
                    }
            ...
            {
                Logger.Error("DCL.UUID", $"MachineGuidDCLGen: {e.Message}");
            }
        ...
        private static (bool, string) GetCachedHexUUID()
        {
            try
            {
                ...
                {
                    Logger.Info("DCL.UUID", $"Returning valid GUID from file '{lastSavedGuid}'");
                    return (true, lastSavedGuid);
                }
                else
                {
                    Logger.Warn("DCL.UUID", $"Read invalid GUID from file '{lastSavedGuid}'");
                }
            }
            catch (Exception e)
            {
                Logger.Error("DCL.UUID", $"Error while reading saved GUID: {e.Message}");
            }
            return (false, "");
        }
        ...
        private static bool CacheHexUUIDToFile(string hexUUID)
        {
            ...
            {
                Logger.Error("DCL.UUID", $"Save GUID fallback failed: {ex.Message}");
                saved = false;
            }
            try
            {
            ...
            {
                Logger.Error("DCL.UUID", $"Logging failed: {ex.Message}");
            }
            return saved;
        }
        ...
        private static string GetCpuID()
        {
            ...
            {
                Logger.Error("DCL.UUID", $"Error GetCpuID(): {e.Message}");
            }
            return serial;
        }
        ...
        private static string GetExtraRigSeed()
        {
            ...
            {
                Logger.Error("DCL.UUID", $"Error GetExtraRigSeed(): {e.Message}");
            }
            return "";
        }
    }
