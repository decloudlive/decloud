using DCL.Common;
using DCL.DeviceMonitoring.AMD;
...
namespace DCL.DeviceMonitoring
{
    internal class DeviceMonitorAMD : DeviceMonitor, IFanSpeedRPM, IGetFanSpeedPercentage, ILoad, IPowerUsage, ITemp, ITDP
    {
        ...
        {
            int percentage = 0;
            int ok = AMD_ODN.DCL_amd_device_get_fan_speed_percentage(BusID, ref percentage);
            if (ok != 0) Logger.InfoDelayed(LogTag, $"DCL_amd_device_get_fan_speed_rpm failed with error code {ok}", _delayedLogging);
        ...
        public int FanSpeedRPM
        {
            ...
            {
                int rpm = 0;
                int ok = AMD_ODN.DCL_amd_device_get_fan_speed_rpm(BusID, ref rpm);
                if (ok == 0) return rpm;
                Logger.InfoDelayed(LogTag, $"DCL_amd_device_get_fan_speed_rpm failed with error code {ok}", _delayedLogging);
                return -1;
            }
        }
        ...
        public float Temp
        {
            ...
            {
                int temperature = 0;
                int ok = AMD_ODN.DCL_amd_device_get_temperature(BusID, ref temperature);
                if (ok == 0) return (float)temperature / 1000;
                Logger.InfoDelayed(LogTag, $"DCL_amd_device_get_temperature failed with error code {ok}", _delayedLogging);
                return -1;
            }
        }
        ...
        public float Load
        {
            ...
            {
                int load_perc = 0;
                int ok = AMD_ODN.DCL_amd_device_get_load_percentage(BusID, ref load_perc);
                if (ok == 0) return load_perc;
                Logger.InfoDelayed(LogTag, $"DCL_amd_device_get_load_percentage failed with error code {ok}", _delayedLogging);
                return -1;
            }
        }
        ...
        public double PowerUsage
        {
            ...
            {
                int power_usage = 0;
                int ok = AMD_ODN.DCL_amd_device_get_power_usage(BusID, ref power_usage);
                if (ok == 0) return power_usage;
                Logger.InfoDelayed(LogTag, $"DCL_amd_device_get_power_usage failed with error code {ok}", _delayedLogging);
                return -1;
            }
        }
        ...
        private bool SetTdpADL(double percValue)
        {
            ...
            int ok = AMD_ODN.DCL_amd_device_get_tdp_min_max_default(BusID, ref min, ref max, ref defaultValue);
            if (ok != 0)
            {
                Logger.InfoDelayed(LogTag, $"DCL_amd_device_get_tdp_ranges failed with error code {ok}", _delayedLogging);
                return false;
            }
            ...
            int ok2 = AMD_ODN.DCL_amd_device_set_tdp(BusID, (int)limit);
            if (ok2 != 0)
            {
                Logger.InfoDelayed(LogTag, $"DCL_amd_device_set_tdp failed with error code {ok}", _delayedLogging);
                return false;
            }
            return true;
        }
        ...
        public double TDPPercentage
        {
            ...
            {
                int tdpRaw = 0;
                int ok = AMD_ODN.DCL_amd_device_get_tdp(BusID, ref tdpRaw);
                if (ok != 0)
                {
                    Logger.InfoDelayed(LogTag, $"DCL_amd_device_get_tdp failed with error code {ok}", _delayedLogging);
                    return -1;
                }
                int min = 0, max = 0, defaultValue = 0;
                int ok2 = AMD_ODN.DCL_amd_device_get_tdp_min_max_default(BusID, ref min, ref max, ref defaultValue);
                if (ok2 != 0)
                {
                    Logger.InfoDelayed(LogTag, $"DCL_amd_device_get_tdp_ranges failed with error code {ok}", _delayedLogging);
                    return -1;
