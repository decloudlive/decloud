...
using DCL.DeviceMonitoring.NVIDIA;
using System;
...
#warning AMD monitoring will attempt to run on each adapter hence the delay log fix this inside AMD monitoring library
        ...
        public static Task<List<DeviceMonitor>> GetDeviceMonitors(IEnumerable<BaseDevice> devices)
        {
            return Task.Run(() =>
            {
                ...
                if (amds.Count > 0)
                {
                    AMD_ODN.DCL_amd_set_debug_log_level(_amdDebugLogLevel);
                    AMD_ODN.DCL_amd_reg_log_cb(_amdLog);
                    var amdInit = AMD_ODN.DCL_amd_init();
                    if (0 == amdInit)
                    {
                        foreach (var amd in amds)
                        {
                            var hasRet = AMD_ODN.DCL_amd_has_adapter(amd.PCIeBusID);
                            if (0 == hasRet)
                            {
                            ...
                            {
                                Logger.Info("DeviceMonitorManager", $"AMD DCL_amd_has_adapter {hasRet} for BusID {amd.PCIeBusID}");
                            }
                        }
                    }
                    else
                    {
                        Logger.Info("DeviceMonitorManager", $"AMD DCL_amd_init {amdInit}");
                    }
                }
                if (nvidias.Count > 0)
                {
