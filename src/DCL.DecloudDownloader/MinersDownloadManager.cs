...
namespace DCL.DecloudDownloader
{
    public static class DecloudDownloadManager
    {
        // don't use this it is faster but less stable
        public static bool UseMyDownloader { get; set; } = true;

        static DecloudDownloadManager()
        {
        ...
        internal static async Task<(bool success, string downloadedFilePath)> DownlaodWithMyDownloaderAsync(string url, string downloadFileRootPath, string fileNameNoExtension, IProgress<int> progress, CancellationToken stop)
        {
            ...
            var timer = new Timer((object stateInfo) =>
            {
                ...
                {
                    Logger.Info("DecloudDownloadManager", $"Error occured while downloading: {downloader.LastError.Message}");
                }
                ...
                {
                    Logger.Debug("DecloudDownloadManager", "Maximum ticks reached, retrying");
                    ticksSinceUpdate = 0;
                }
                else
                {
                    Logger.Debug("DecloudDownloadManager", $"No progress in ticks {ticksSinceUpdate}");
                    ticksSinceUpdate++;
                }
            });
            timer.Change(0, 500);
            ...
            var onDownloadEnded = new EventHandler<DownloaderEventArgs>((object sender, DownloaderEventArgs e) =>
            {
                ...
                if (downloader != null)
                {
                    ...
                    {
                        Logger.Info("DecloudDownloadManager", $"Download didn't complete successfully: {downloader.LastError.Message}");
                        tcs.SetResult(false);
                    }
                    else if (downloader.State == DownloaderState.Ended)
                    {
                        Logger.Info("DecloudDownloadManager", "DownloadCompleted Success");
                        tcs.SetResult(true);
                    }
                }
            });
        ...
        internal static async Task<(bool success, string downloadedFilePath)> DownlaodWithMegaFileAsync(string url, string downloadFileRootPath, string fileNameNoExtension, IProgress<int> progress, CancellationToken stop)
        {
            ...
            {
                Logger.Error("DecloudDownloadManager", $"MegaFile error: {e.Message}");
            }
            finally
            {
                client.Logout();
        ...
        internal static async Task<(bool success, string downloadedFilePath)> DownlaodWithMegaFromFolderAsync(string url, string downloadFileRootPath, string fileNameNoExtension, IProgress<int> progress, CancellationToken stop)
        {
            ...
            {
                Logger.Error("DecloudDownloadManager", $"MegaFolder error: {e.Message}");
            }
            finally
            {
                client.Logout();
        ...
        private static async Task<bool> UrlIsValid(string url)
        {
            try
            {
                ...
                using (var response = await request.GetResponseAsync() as HttpWebResponse)
                {
                    ...
                    {
                        Logger.Info("DecloudDownloadManager", $"The remote server has thrown an internal error. Url is not valid: {url}");
                        return false;
                    }
                }
            }
            catch (WebException ex)
            {
                ...
                {
                    Logger.Error("DecloudDownloadManager", $"Unhandled status [{ex.Status}] returned for url: {url}. Ex {ex.Message}");
                }
            }
            catch (Exception ex)
            {
                Logger.Error("DecloudDownloadManager", $"Could not test url {url}. Ex {ex}");
            }
            return false;
        }
    }
