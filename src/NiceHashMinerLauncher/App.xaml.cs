...
namespace Decloud
{
    ...
    public partial class App : Application
    {
        ...
        static readonly string[] AppExeNames = new string[] { "Decloud.exe", "app_DCL.exe" };
        ...
        private static string GetLatestUpdater()
        {
            ...
            const string updaterPrefix = "DCL_windows_";
        ...
        private async void App_OnStartup(object sender, StartupEventArgs e)
        {
            ...
            if (isUpdater)
            {
                ...
                if (latestUpdaterFile == null)
                {
                    var restartProcessUpdatefailed = new Process
                    {
                        ...
                        {
                            FileName = GetRootPath("Decloud.exe"),
                ...
                else
                {
                    ...
                    startUpdater.Start();
                    // TODO we should wait and check if we have restarted Decloud
                }
            ...
            try
            {
                ...
                if (!createdNew)
                {
                    //MessageBox.Show("We have detected you are already running Decloud. Only a single instance should be running at a time.", "Decloud Warning", MessageBoxButton.OK, MessageBoxImage.Warning);
                    // shutdown
                    Shutdown();
                    return;
                }
...
#error "Comment this line if you really wish to execute apps cleanup"
...
#endif
                var DCLApp = GetRootPath(latestAppDir, latestAppExe);
                ...
                while (run)
                {
                    ...
                    try
                    {
                        using (var Decloud = new Process { StartInfo = startInfo })
                        {
                            var hasStarted = Decloud?.Start();
                            Decloud?.WaitForExit();
                            // TODO 
                            Console.WriteLine(Decloud.ExitCode);
                            //in case of crash try to restart the program
                            if (Decloud.ExitCode != 0)
                            {
                                ...
                                if (!File.Exists(path))
                                {
                                    ...
                                    {
                                        sw.WriteLine($"Exit code: {Decloud.ExitCode} ---- {endTime}");
                                    }
                                }
                                else
                                {
                                ...
                                if (restartCount >= maxRestartCount)
                                {
                                    using (var sw = File.AppendText(path))
                                    {
                                        sw.WriteLine($"Too many restarts! Closing DCL");
                                    }
                                    MessageBox.Show("DCL experienced too many crashes recently, therefore it will close itself", "Too many restarts");
                                    run = false;
                                }
                            }
                            else
                            {
                                ...
                                else if (IsUpdate())
                                {
                                    ...
                                    var tmpLauncher = GetRootPath($"tmp.DCL_updater_{randomPart}.exe");
