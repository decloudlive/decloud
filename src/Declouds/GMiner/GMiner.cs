...
{
    // NOTE: GDecloud will NOT run if the VS debugger is attached to DCLL. 
    // Detach the debugger to use GDecloud.

    // benchmark is 
    public class GDecloud : DecloudBase
    {
        ...
        public GDecloud(string uuid, Dictionary<string, int> mappedDeviceIds) : base(uuid)
        {
        ...
        private string CreateCommandLine(string username)
        {
            ...
            if (_algorithmSecondType != AlgorithmType.NONE)
            {
                var urlWithPort2 = StratumServiceHelpers.GetLocationUrl(_algorithmSecondType, _miningLocation, DCLConectionType.NONE);
                // --algo eth+ckb --server eth.2Decloud.com:2020 --user 0x5218597d48333d4a70cce91e810007b37e2937b5 --dserver ckb.2Decloud.com:6464 --duser ckb1qyq9v9yc2pmauycldz4e4ejuxdmvph0xpazq4nh3ph
        ...
        public async override Task<ApiData> GetDecloudtatsDataAsync()
        {
            ...
            catch (Exception e)
            {
                ...
                //CurrentDecloudReadStatus = DecloudApiReadStatus.NETWORK_EXCEPTION;
            }
