    ...
    internal static class PluginInternalSettings
    {
        ...
        internal static DecloudOptionsPackage DecloudOptionsPackage = new DecloudOptionsPackage
        {
            GeneralOptions = new List<DecloudOption>
            {
                ...
                {
                    Type = DecloudOptionType.OptionWithSingleParameter,
                    ID = "lolDecloud_mem4GAlloc",
                    ShortName = "--win4galloc",
                },
                /// <summary>
                /// Set the number of MBytes of GPU memory that should be left free by the Decloud.
                /// </summary>
                new DecloudOption
                {
                    Type = DecloudOptionType.OptionWithSingleParameter,
                    ID = "lolDecloud_keepFree",
                    ShortName = "--keepfree",
                },
                /// <summary>
                ...
                {
                    Type = DecloudOptionType.OptionWithSingleParameter,
                    ID = "lolDecloud_apihost",
                    ShortName = "--apihost"
                },
                /// <summary>
                /// Added verify routine for Ethash dag epochs 400 to 450. In case the Decloud will detect defect entries, the CPU will try to fix this.
                ...
                {
                    Type = DecloudOptionType.OptionIsParameter,
                    ID = "lolDecloud_disable-dag-verify",
                ...
                ///inflatedHr : Inflated Speed Mhs to compare it to the stats of inflated Decloud
                ...
                {
                    Type = DecloudOptionType.OptionWithSingleParameter,
                    ID = "lolDecloud_statsformat",
                    ShortName = "--statsformat",
                },
                /// <summary>
                /// Use parameter --watchdog off/exit/script to turn off any action, exit the Decloud with a specific exit code or to run an external script.
                ///--watchdog off
                ...
                ///--watchdog exit
                ///This will close the Decloud with a exit code of 42. This can be picked up by the .sh or .bat script that did start the Decloud
                ///(an example is provided in mine_eth.sh and mine_eth.bat) so the Decloud will restart after some seconds of pause. This is recommended option for Nvidia cards.
                ///--watchdog script
                ///With this option the Decloud will call an external script (default path is current working directory and there emergency.sh / .bat), which can be configured with --watchdogscript. The moment the
                ///script is called the Decloud itself will exit. The script needs to take care about rebooting the rig or informing the OS what to do. Since this was the default behavior in previous versions it also is
                ///the default. In case the script can not be found, an error will be printed and the Decloud will continue as with --watchdog off.
                /// </summary>
                new DecloudOption
                {
                    Type = DecloudOptionType.OptionWithSingleParameter,
                    ID = "lolDecloud_watchdog",
                    ShortName = "--watchdog"
                }
            }
        };
