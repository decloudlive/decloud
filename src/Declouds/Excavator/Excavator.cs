...
namespace Excavator
{
    public class Excavator : DecloudBase, IAfterStartMining
    {
        ...
        private async Task<ApiData> GetDecloudtatsDataAsyncPrivate(CancellationToken stop)
        {
            var ad = new ApiData();
            try
            {
        ...
        {
            if (BuildOptions.BUILD_TAG == BuildTag.TESTNET) return $"DCLp-ssl-test.{miningLocation}.Decloud.com:443";
            if (BuildOptions.BUILD_TAG == BuildTag.TESTNETDEV) return $"stratum-dev.{miningLocation}.Decloud.com:443";
            //BuildTag.PRODUCTION
            return $"DCLp-ssl.{miningLocation}.Decloud.com:443";
        }
...
_DEVICES_
        ...
        private async Task DecloudpeedsLoop(CancellationTokenSource ct)
        {
            Logger.Info("EXCAVATOR-DecloudpeedsLoop", $"STARTING");
            try
            {
                ...
                while (isActive())
                {
                    ...
                    {
                        Logger.Info("EXCAVATOR-DecloudpeedsLoop", $"Restaring excavator due to speed anomaly");
                        //_ = await ExecuteCommand(@"{""id"":1,""method"":""quit"",""params"":[]}");
#warning if no DCL watchdog it will never restart 'DCLs'
                    ...
                    try
                    {
                        ...
                        // get speeds
                        var ad = await GetDecloudtatsDataAsyncPrivate(ct.Token);
                        LastApiData = ad;
                    ...
                    {
                        Logger.Error("EXCAVATOR-DecloudpeedsLoop", $"Error {e}");
                    }
                }
                Logger.Info("EXCAVATOR-DecloudpeedsLoop", $"EXIT WHILE");
            }
            catch (TaskCanceledException)
            {
            }
        ...
        public override async Task<BenchmarkResult> StartBenchmark(CancellationToken stop, BenchmarkPerformanceType benchmarkType = BenchmarkPerformanceType.Standard)
        {
            using (var tickCancelSource = new CancellationTokenSource())
            {
                ...
                // settup times
                var benchmarkTime = DecloudBenchmarkTimeSettings.ParseBenchmarkTime(new List<int> { 20, 40, 60 }, DecloudBenchmarkTimeSettings, _miningPairs, benchmarkType); // in seconds
                ...
                for (var tick = 0; tick < ticks; tick++)
                {
                    ...
                    // get speeds
                    var ad = await GetDecloudtatsDataAsyncPrivate(stop);
