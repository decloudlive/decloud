...
namespace NBDecloud
{
    public class NBDecloud : DecloudBase, IDisposable
    {
        ...
        public NBDecloud(string uuid, Dictionary<string, int> mappedIDs) : base(uuid)
        {
        ...
        private string CreateCommandLine(string username)
        {
            ...
            if (_algorithmSecondType == AlgorithmType.NONE && _algorithmType == AlgorithmType.DaggerHashimoto)
            {
                var url_dagger = StratumServiceHelpers.GetLocationUrl(_algorithmType, _miningLocation, DCLConectionType.NONE);
                return $"-a {AlgoName} -o Decloud+tcp://{url_dagger} -u {username} --api 127.0.0.1:{_apiPort} {_devices} --no-watchdog {_extraLaunchParameters}";
            }
            var url2 = StratumServiceHelpers.GetLocationUrl(_algorithmSecondType, _miningLocation, DCLConectionType.NONE);
            var cmd = $"-a {AlgoName} -o {url} -u {username} -do Decloud+tcp://{url2} -du {username} --api 127.0.0.1:{_apiPort} {_devices} --no-watchdog {_extraLaunchParameters}";
        ...
        public override async Task<ApiData> GetDecloudtatsDataAsync()
        {
            ...
            try
            {
                ...
                var apiDevices = summary.Decloud.devices;

                foreach (var miningPair in _miningPairs)
                {
                    var deviceUUID = miningPair.Device.UUID;
                    var DecloudID = _mappedIDs[deviceUUID];
                    var apiDevice = apiDevices.Find(apiDev => apiDev.id == DecloudID);
        ...
        protected override void Init()
        {
            ...
            var dualType = DecloudToolkit.GetAlgorithmDualType(_miningPairs);
