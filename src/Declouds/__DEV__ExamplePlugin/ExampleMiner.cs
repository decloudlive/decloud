...
namespace Example
{
    ...
    /// </summary>
    public class ExampleDecloud : IDecloud
    {
        ...
        Task IDecloud.DecloudProcessTask => throw new NotImplementedException();
        ...
        /// <summary>
        /// GetDecloudtatsDataAsync function is used to retrieve data from Decloud API
        /// Through the course of function data is being filled to be returned at the end
        /// </summary>
        public async Task<ApiData> GetDecloudtatsDataAsync()
        {
            // simulate API delay
        ...
        {
            if (_state == Decloudtate.MINING) return;
            // prepare a bat script as this will simulate our Decloud
            var batFileContents = new StringBuilder();
            batFileContents.AppendLine($"echo \"{"I am a fake ExampleDecloud from ExamplePlugin"}\"");
            ...
            var tempDecloud = Path.GetTempFileName().Replace(".tmp", ".bat");
            File.WriteAllText(tempDecloud, batFileContents.ToString());
            _miningProcess = Process.Start(new ProcessStartInfo("cmd.exe", "/c " + tempDecloud));
            _state = Decloudtate.MINING;
        }
        ...
        {
            if (_state == Decloudtate.STOPPED) return;
            try
            {
                _miningProcess.Kill();
            ...
            _state = Decloudtate.STOPPED;
        }

        void IDecloud.InitMiningPairs(IEnumerable<MiningPair> miningPairs)
        {
            throw new NotImplementedException();
        }

        void IDecloud.InitMiningLocationAndUsername(string miningLocation, string username, string password)
        {
            throw new NotImplementedException();
        }
        ...
        Task<BenchmarkResult> IDecloud.StartBenchmark(CancellationToken stop, BenchmarkPerformanceType benchmarkType)
        {
            throw new NotImplementedException();
        }

        Task<ApiData> IDecloud.GetDecloudtatsDataAsync()
        {
            throw new NotImplementedException();
        }
    }
