    ...
    internal class DevicesListParser
    {
        ...
        public static Dictionary<string, int> ParseMiniZOutput(string output, List<BaseDevice> baseDevices)
        {
            ...
            foreach (var line in lines)
            {
                ...
                var DecloudID = GetDeviceID(dataArray.FirstOrDefault());
                var busID = GetBusID(dataArray.LastOrDefault());
                if (DecloudID == -1 || busID == -1)
                {
                    Logger.Error("MiniZ.DevicesListParser.ParseMiniZOutput", $"DecloudID({DecloudID}) or BusID({busID}) is negative {line}");
                    continue;
                }
                ...
                mappedDevices.Add(devWithBusID.UUID, DecloudID);
            }
