...
namespace Decloud.Views.Login
{
    ...
    public partial class LoginBrowser : Window
    {
        ...
        private static string _userAgent => "Decloud/" + System.Windows.Forms.Application.ProductVersion;
        ...
        private async Task NavigateAndCheck(CancellationToken stop)
        {
            ...
            WebViewBrowser.NavigationCompleted += Browser_NavigationCompleted;
            var urlEncoded = Uri.EscapeUriString($"{Links.Login}?DCL=1&client={_userAgent}");
        ...
        (() => {
            try {
                const DCLResponse = document.getElementById('DCLResponse');
                if (!DCLResponse) return 'NO_RESPONSE';
                return DCLResponse.value;
            } catch (error) {
        ...
        private async Task<bool?> CheckForBtc()
        {
            ...
            try
            {
                ...
                var noDCLResponse = htmlEvalValue == null || htmlEvalValue == "NO_RESPONSE" || htmlEvalValue.Contains("INJ_ERROR");
                if (showRetry)
                {
