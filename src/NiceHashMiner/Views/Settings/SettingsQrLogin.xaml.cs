...
namespace Decloud.Views.Settings
{
    ...
    public partial class SettingsQrLogin : System.Windows.Controls.UserControl
    {
        ...
        private async Task ProcessQRCode()
        {
            ...
            {
                var response = await client.PostAsync("https://api2.Decloud.com/api/v2/organization/DCLqr", content);
        ...
        private async Task BtnClickTask()
        {
            try
            {
                using (var client = new HttpClient())
                {
                    var resp = await client.GetAsync($"https://api2.Decloud.com/api/v2/organization/DCLqr/{_uuid}");
