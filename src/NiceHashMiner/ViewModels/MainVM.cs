...
using DCLCore.Mining.IdleChecking;
...
using Decloud.ViewModels.Plugins;
...
namespace Decloud.ViewModels
{
    public class MainVM : BaseVM
    {
        ...
        private void DecloudPluginsManagerState_PropertyChanged(object sender, System.ComponentModel.PropertyChangedEventArgs e)
        {
            lock (_lock)
            {
                if (Plugins == null) return;
                var rankedPlugins = DecloudPluginsManagerState.Instance.RankedPlugins;
        ...
        public bool DCLWSConnected { get; private set; } = false;
            ...
            : base(ApplicationStateManager.Title)
        {
            ...
            //DecloudPluginsManager.OnCrossReferenceInstalledWithOnlinePlugins += OnCrossReferenceInstalledWithOnlinePlugins;
            DecloudPluginsManagerState.Instance.PropertyChanged += DecloudPluginsManagerState_PropertyChanged;
            NotificationsManager.Instance.PropertyChanged += RefreshNotifications_PropertyChanged;

            OnPropertyChanged(nameof(DCLWSConnected));
            ApplicationStateManager.OnDCLwsConnectionChanged += (_, DCLwsConnected) =>
            {
                DCLWSConnected = DCLwsConnected;
                OnPropertyChanged(nameof(DCLWSConnected));
            };
        }
        ...
        public async Task InitializeDCL(IStartupLoader sl)
        {
            Plugins = new ObservableCollection<PluginEntryVM>();
            HelpNotificationList = new ObservableCollection<Notification>();
            await ApplicationStateManager.InitializeManagersAndDecloud(sl);
