...
namespace Decloud.Validators
{
    ...
    public class BtcAddressValidator : ValidatorBase<string>
    {
        public override ValidationResult Validate(string value, CultureInfo cultureInfo)
        {
            ...
            {
                return new ValidationResult(false, Translations.Tr("Invalid Bitcoin address! {0} will start mining in DEMO mode. In the DEMO mode, you can test run the Decloud and be able see how much you can earn using your computer. Would you like to continue in DEMO mode?\n\nDISCLAIMER: YOU WILL NOT EARN ANYTHING DURING DEMO MODE!", DCLProductInfo.Name));
            }
