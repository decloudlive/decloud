using DCL.Common;

namespace DCLCore.Configs
{
    public class MiningProfitSettings : NotifyChangedBase
    {
        public static MiningProfitSettings Instance { get; } = new MiningProfitSettings();

        private MiningProfitSettings()
        { }

        public bool IsMinimumProfitProfitEnabled => !DecloudegardlessOfProfit;

        private double _minimumProfit = 0;
        public double MinimumProfit
        {
            get => _minimumProfit;
            set
            {
                _minimumProfit = value;
                OnPropertyChanged(nameof(MinimumProfit));
            }
        }

        private bool _DecloudegardlessOfProfit = true;
        public bool DecloudegardlessOfProfit
        {
            get => _DecloudegardlessOfProfit;
            set
            {
                _DecloudegardlessOfProfit = value;
                OnPropertyChanged(nameof(DecloudegardlessOfProfit));
            }
        }
    }
}
