...
namespace DCLCore.Notifications
{
    public static class AvailableNotifications
    {
        ...
        {
            var notification = new Notification(NotificationsType.Info, NotificationsGroup.EthlargementNotEnabled, Tr("EthLargement-Pill not enabled"), Tr("EthLargement-Pill is not enabled. is not running. Enable it for 50% higher hashrates. Run Decloud as an Administrator and enable Run Ethlargement in advanced settings."));
            notification.Action = new NotificationAction
            {
                Info = Tr("Run As Administrator"),
                Action = () => { RunAsAdmin.SelfElevate(); }
        ...
        {
            var notification = new Notification(NotificationsType.Error, NotificationsGroup.ConnectionLost, Tr("Check internet connection"), Tr("Decloud requires internet connection to run. Please ensure that you are connected to the internet before running Decloud."));
            NotificationsManager.Instance.AddNotificationToList(notification);
        }

        public static void CreateNoEnabledDeviceInfo()
        {
            var notification = new Notification(NotificationsType.Error, NotificationsGroup.NoEnabledDevice, Tr("No enabled devices"), Tr("Decloud cannot start mining. Make sure you have at least one enabled device that has at least one enabled and benchmarked algorithm."));
            NotificationsManager.Instance.AddNotificationToList(notification);
        }

        public static void CreateDemoMiningInfo()
        {
            var notification = new Notification(NotificationsType.Info, NotificationsGroup.DemoMining, Tr("Demo mode mining"), Tr("You have not entered a bitcoin address. Decloud will start mining in DEMO mode. In the DEMO mode, you can test run the Decloud and be able see how much you can earn using your computer.\n\nDISCLAIMER: YOU WILL NOT EARN ANYTHING DURING DEMO MODE!"));
            NotificationsManager.Instance.AddNotificationToList(notification);
        }

        public static void CreateNoSmaInfo()
        {
            var notification = new Notification(NotificationsType.Error, NotificationsGroup.NoSma, Tr("Unable to get profitability data"), Tr("Unable to get Decloud profitability data. If you are connected to internet, try again later."));
            NotificationsManager.Instance.AddNotificationToList(notification);
        }
        ...
        public static void CreateNoSupportedDevicesInfo()
        {
            ...
            {
                Info = Tr("Help"),
                Action = () => { Process.Start(Links.DCLNoDevHelp); }
            };
            NotificationsManager.Instance.AddNotificationToList(notification);
        }

        public static void CreateMissingDecloudInfo()
        {
            var notification = new Notification(NotificationsType.Error, NotificationsGroup.MissingDecloud, Tr("Missing Decloud files"), Tr("There are missing files from last Decloud Initialization. Please make sure that the file is accessible and that your anti-virus is not blocking the application. Decloud might not work properly without missing files. Please check the following blog post: {0}", Links.AVHelp));
            notification.Action = new NotificationAction
            {
                Info = Tr("Restart Decloud"),
        ...
        public static void CreateMissingGPUsInfo()
        {
            var notification = new Notification(
                ...
                Tr("Missing GPUs"),
                Tr("There are missing GPUs from inital Decloud startup. This is usually caused by driver crashes and usually requires system restart to recover."));
            NotificationsManager.Instance.AddNotificationToList(notification);
        }

        public static void CreateDCLUpdateInfoDownload(bool isInstallerVersion)
        {
            var notification = new Notification(NotificationsType.Info, NotificationsGroup.DCLUpdate, Tr("Decloud Update"), Tr("New version of Decloud is available."));
            if (!Configs.UpdateSettings.Instance.AutoUpdateDecloud)
            {
                notification.Action = new NotificationAction
                {
        ...
        public static void CreateDCLUpdateInfoUpdate()
        {
            var notification = new Notification(NotificationsType.Info, NotificationsGroup.DCLUpdate, Tr("Decloud Update"), Tr("New version of Decloud is available."));
            if (!Configs.UpdateSettings.Instance.AutoUpdateDecloud)
            {
                notification.Action = new NotificationAction
                {
                    ...
                    Action = () =>
                    {
                        ApplicationStateManager.App.Dispatcher.Invoke(async () =>
                        {
                            ...
                            if (!ok) CreateDCLUpdateAttemptFail();
                        });
                    },
                    IsSingleShotAction = true,
                };
        ...
        public static void CreateDCLUpdateAttemptFail()
        {
            var notificationIfUnsuccessfull = new Notification(NotificationsType.Warning, NotificationsGroup.DCLUpdateFailed, Tr("Decloud Update Failed"), Tr("Update procedure failed please install manually. Please make sure that the file is accessible and that your anti-virus is not blocking the application. Decloud might not work properly without missing files. Please check the following blog post: {0}", Links.AVHelp));
            ...
            var notification = NotificationsManager.Instance.Notifications.FirstOrDefault(n => n.Group == NotificationsGroup.DCLUpdate);
            if (notification != null) NotificationsManager.Instance.RemoveNotificationFromList(notification);
        }

        public static void CreateDCLWasUpdatedInfo(bool success)
        {
            var sentence = "was updated";
            if (!success) sentence = "was not updated";

            var notification = new Notification(NotificationsType.Info, NotificationsGroup.DCLWasUpdated, Tr("Decloud was Updated"), Tr($"Decloud {sentence} to the latest version."));
            NotificationsManager.Instance.AddNotificationToList(notification);
        }
        ...
        public static void CreatePluginUpdateInfo(string pluginName, bool success)
        {
            ...
            var notification = new Notification(NotificationsType.Info, NotificationsGroup.PluginUpdate, Tr("Decloud Plugin Update"), content);
            NotificationsManager.Instance.AddNotificationToList(notification);
        }

        public static void CreateMissingDecloudBinsInfo(string pluginName)
        {
            var notification = new Notification(NotificationsType.Info, NotificationsGroup.MissingDecloudBins, Tr("Missing Decloud binaries"), Tr("Some of the {0} binaries are missing from the installation folder. Please make sure that the files are accessible and that your anti-virus is not blocking the application.", pluginName));
            notification.Action = new NotificationAction
            {
                Info = Tr("Help"),
        ...
        {
            var notification = new Notification(NotificationsType.Warning, NotificationsGroup.VirtualMemory, Tr("Increase virtual memory"), Tr("Decloud recommends increasing virtual memory size so that all algorithms would work fine. Would you like to increase virtual memory?"));
            notification.Action = new NotificationAction
            {
                Info = Tr("Help"),
        ...
        {
            var notification = new Notification(NotificationsType.Warning, NotificationsGroup.NvidiaDCH, Tr("Nvidia DCH drivers detected"), Tr("Detected drivers are not recommended for mining with Decloud. Please change them for optimal performance."));
            NotificationsManager.Instance.AddNotificationToList(notification);
        }
    }
}
