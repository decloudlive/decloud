...
namespace DCLCore.Utils
{
    public static class UpdateHelpers
    {
        ...
        public static void StartLoops(CancellationToken stop)
        {
            ...
            {
                var loop1 = DCLAutoUpdateCheckLoop(stop);
                return Task.WhenAll(loop1);
            });
        }

        private static async Task DCLAutoUpdateCheckLoop(CancellationToken stop)
        {
            try
            {
                ...
                while (isActive())
                {
                    ...
                    var isAutoUpdate = UpdateSettings.Instance.AutoUpdateDecloud;
                    var hasNewVersion = VersionState.Instance.IsNewVersionAvailable;
                    // prevent sleep check

                    bool isUpdater = IsDCLInstalled() && IsRunningInstalledApp();
                    if (hasNewVersion)
                    {
                        AvailableNotifications.CreateDCLUpdateInfoDownload(isUpdater);
                    }

                    if (isActive() && isAutoUpdate && hasNewVersion && !Launcher.IsUpdatedFailed)
                    {
                        var ok = await StartAutoUpdateProcess(isUpdater);
                        if (!ok) AvailableNotifications.CreateDCLUpdateAttemptFail();
                    }
                }
            }
            catch (TaskCanceledException e)
            {
                Logger.Info(Tag, $"DCLAutoUpdateCheckLoop TaskCanceledException: {e.Message}");
            }
            catch (Exception e)
            {
                Logger.Error(Tag, $"DCLAutoUpdateCheckLoop Exception: {e.Message}");
            }
            finally
            {
                Logger.Info(Tag, "Exiting DCLAutoUpdateCheckLoop run cleanup");
                // cleanup
            }
        }
        ...
        internal static async Task<bool> StartDownloadingUpdater(bool isUpdater)
        {
            try
            {
                ...
                var updateNotification = NotificationsManager.Instance.Notifications.Find(notif => notif.Group == NotificationsGroup.DCLUpdate);
                ...
                var saveAsFile = isUpdater ? $"DCL_windows_updater_{VersionState.Instance.OnlineVersionStr}" : $"DCL_windows_{VersionState.Instance.OnlineVersionStr}";
                var downloadProgress = updateNotification?.Action?.Progress ?? null;
                var (success, downloadedFilePath) = await DecloudDownloadManager.DownloadFileWebClientAsync(url, downloadRootPath, saveAsFile, downloadProgress, ApplicationStateManager.ExitApplication.Token);
                if (!success)
                {
        ...
        internal static async Task<bool> StartUpdateProcess()
        {
            try
            {
                ...
                if (Launcher.IsLauncher)
                {
                    ...
                    {
                        Logger.Error("Decloud", $"Autoupdate IsLauncher error: {e.Message}");
        ...
        public static bool IsDCLInstalled()
        {
