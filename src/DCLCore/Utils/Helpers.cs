...
namespace DCLCore.Utils
{
    public class Helpers : PInvokeHelpers
    {
        ...
        public static void DisableWindowsErrorReporting(bool en)
        {
            ...
            Logger.Info("Decloud", "Trying to enable/disable Windows error reporting");
            ...
            try
            {
                using (var rk = Registry.CurrentUser.CreateSubKey(@"Software\Microsoft\Windows\Windows Error Reporting"))
                {
                    if (rk != null)
                    {
                        ...
                        {
                            var val = (int)o;
                            Logger.Info("Decloud", $"Current DontShowUI value: {val}");

                            if (val == 0 && en)
                            {
                                Logger.Info("Decloud", "Setting register value to 1.");
                                rk.SetValue("DontShowUI", 1);
                            }
                            else if (val == 1 && !en)
                            {
                                Logger.Info("Decloud", "Setting register value to 0.");
                                rk.SetValue("DontShowUI", 0);
                            }
                        }
                        else
                        {
                            Logger.Info("Decloud", "Registry key not found .. creating one..");
                        ...
                        Logger.Info("Decloud", "Unable to open SubKey.");
                }
            }
            catch (Exception ex)
            {
                Logger.Error("Decloud", $"Unable to access registry. Error: {ex.Message}");
            }
        }
        ...
        public static void SetNvidiaP0State()
        {
            try
            {
                ...
                using (var p = new Process { StartInfo = startInfo })
                {
                    ...
                    if (p?.ExitCode != 0)
                        Logger.Info("Decloud", "nvidiasetp0state returned error code: " + p.ExitCode);
                    else
                        Logger.Info("Decloud", "nvidiasetp0state all OK");
                }
            }
            catch (Exception ex)
            {
                Logger.Error("Decloud", "nvidiasetp0state error: " + ex.Message);
            }
        }
        ...
        public static void VisitUrlLink(string urlLink)
        {
            ...
            {
                Logger.Error("Decloud", "VisitLink error: " + ex.Message);
            }
        }
