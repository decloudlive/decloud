    ...
    public static class RunAsAdmin
    {
        ...
        public static void Execute(int pid, string path)
        {
            if (Launcher.IsLauncher)
            {
                ...
                {
                    Logger.Error("Decloud", $"RunAsAdmin IsLauncher error: {e.Message}");
                }
            }
            else
            {
                try
                {
                    var fileName = Paths.RootPath("runDCLasadmin.exe");
                ...
                {
                    Logger.Error("Decloud", $"RunAsAdmin error: {ex.Message}");
                }
            }
        }
    }
