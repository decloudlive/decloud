...
namespace DCLCore
{
    ...
    internal class DecloudStats
    {
#pragma warning disable 649, IDE1006

        #region JSON Models

        private class Decloud_login
        {
        ...
        private class Decloud_credentials
        {
        ...
        private class Decloud_device_status
        {
            public string method = "devices.status";
            public List<JArray> devices;
        }

        private class DecloudExchange
        {
...
#pragma warning restore 649, IDE1006
        ...
        public static Dictionary<AlgorithmType, DecloudSma> AlgorithmRates { get; private set; }
        private static DecloudData _DecloudData;
        public static double Balance { get; private set; }
        public static string Version { get; private set; }
        public static bool IsAlive => DecloudConnection.IsAlive;
        ...
        private class DecloudConnection
        {
            ...
            private static void ConnectCallback(object sender, EventArgs e)
            {
                try
                {
                    if (AlgorithmRates == null || _DecloudData == null)
                    {
                        _DecloudData = new DecloudData();
                        AlgorithmRates = _DecloudData.NormalizedSma();
                    }
                    //send login
                    var version = "DCLL/" + Application.ProductVersion;
                    var login = new Decloud_login
                    {
                        version = version
                    };
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            StartConnection(Links.DCLSocketAddress);
                        }
                    }
                }
                catch (Exception e)
        ...
        public static void StartConnection(string address)
        {
            DecloudConnection.StartConnection(address);
        ...
        private static void SetAlgorithmRates(JArray data)
        {
            try
            {
                foreach (var algo in data)
                {
                    ...
                    _DecloudData.AppendPayingForAlgo(algoKey, algo[1].Value<double>());
                }
                AlgorithmRates = _DecloudData.NormalizedSma();
        ...
        {
            DecloudExchange exchange = null;
            try
            {
                exchange = JsonConvert.DeserializeObject<DecloudExchange>(data);
            }
            catch { }
        ...
        {
            var data = new Decloud_credentials
            {
                btc = btc,
                worker = worker
            };
            if (BitcoinAddress.ValidateBitcoinAddress(data.btc) && BitcoinAddress.ValidateWorkerName(worker))
            {
                ...
                Task.Factory.StartNew(() => DecloudConnection.SendData(sendData));
            }
        }
        ...
        public static void DeviceStatus_Tick(object state)
        {
            ...
            var activeIDs = DecloudManager.GetActiveDecloudIndexes();
            ...
            var data = new Decloud_device_status
            {
                devices = deviceList
            };
            var sendData = JsonConvert.SerializeObject(data);
            ...
            DecloudConnection.SendData(sendData);
        }

        #endregion

        public static string GetDecloudApiData(string url, string worker)
        {
            var responseFromServer = "";
            try
            {
                var activeDecloudGroup = DecloudManager.GetActiveDecloudGroup();

                var wr = (HttpWebRequest) WebRequest.Create(url);
                wr.UserAgent = "Decloud/" + Application.ProductVersion;
                if (worker.Length > 64) worker = worker.Substring(0, 64);
                wr.Headers.Add("Decloud-Worker-ID", worker);
                wr.Headers.Add("DCL-Active-Decloud-Group", activeDecloudGroup);
                wr.Timeout = 30 * 1000;
            ...
            {
                Helpers.ConsolePrint("Decloud", ex.Message);
                return null;
            }
