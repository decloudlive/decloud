...
namespace DCLCore
{
    static partial class ApplicationStateManager
    {
        ...
        private static DelayedSingleExecActionTask _resetDecloudStatsCredentialsDelayed = new DelayedSingleExecActionTask
            (
            ResetDecloudStatsCredentials,
            new TimeSpan(0, 0, 5)
            );

        public static event EventHandler<bool> OnDCLwsConnectionChanged;
        public static void SetDCLwsConnectionChanged(bool isConnected)
        {
            App.Dispatcher.Invoke(() =>
            {
                OnDCLwsConnectionChanged?.Invoke(null, isConnected);
            });
        }


        static void ResetDecloudStatsCredentials()
        {
        ...
        public static async Task<SetResult> SetBTCIfValidOrDifferent(string btc, bool skipCredentialsSet = false)
        {
            ...
            {
                _resetDecloudStatsCredentialsDelayed.ExecuteDelayed(CancellationToken.None);
            }
