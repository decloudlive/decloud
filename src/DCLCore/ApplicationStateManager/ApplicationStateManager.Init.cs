...
using DCL.DeviceDetection;
using DCL.DeviceMonitoring;
...
namespace DCLCore
{
    static partial class ApplicationStateManager
    {
        ...
        public static async Task InitializeManagersAndDecloud(IStartupLoader loader)
        {
            try
            {
                var allSteps = 14;
                ...
                // STEP
                // load plugins
                loader.PrimaryProgress?.Report((Tr("Loading Decloud plugins..."), nextProgPerc()));
                // Plugin Loading
                await DecloudPluginsManager.LoadAndInitDecloudPlugins();
                DecloudPluginsManager.StartLoops(ExitApplication.Token);
                // commit again benchmarks after loading plugins
                ConfigManager.CommitBenchmarks();
                /////////////////////////////////////////////
                /////// from here on we have our devices and Decloud initialized
                MiningState.Instance.CalculateDevicesStateChange();

                // STEP
                // connect to DCLws
                loader.PrimaryProgress?.Report((Tr("Connecting to DCLws..."), nextProgPerc()));
                // Init ws connection
                var (btc, worker, group) = CredentialsSettings.Instance.GetCredentials();
                NHWebSocket.SetCredentials(btc, worker, group);
                NHWebSocket.StartLoop(DCL.Common.DCLws.DCLSocketAddress, ExitApplication.Token);
                ...
                // STEP
                // Update Decloud plugin binaries
                loader.SecondaryTitle = Tr("Updating Decloud Binaries");
                loader.SecondaryVisible = true;
                loader.PrimaryProgress?.Report((Tr("Updating Decloud Binaries..."), nextProgPerc()));
                await DecloudPluginsManager.UpdateDecloudBins(loader.SecondaryProgress, ExitApplication.Token);
                loader.SecondaryVisible = false;
                if (ExitApplication.IsCancellationRequested) return;

                // STEP
                loader.SecondaryTitle = Tr("Downloading Decloud Binaries");
                loader.SecondaryVisible = true;
                loader.PrimaryProgress?.Report((Tr("Downloading Decloud Binaries..."), nextProgPerc()));
                await DecloudPluginsManager.DownloadMissingDecloudBins(loader.SecondaryProgress, ExitApplication.Token);
                loader.SecondaryVisible = false;
                if (ExitApplication.IsCancellationRequested) return;
                ...
                // re-check after download we should have all Decloud files
                if (DecloudPluginsManager.HasMissingDecloud())
                {
                    AvailableNotifications.CreateMissingDecloudInfo();
                }
                ...
                loader.PrimaryProgress?.Report((Tr("Cross referencing Decloud device IDs..."), nextProgPerc()));
                // Detected devices cross reference with Decloud indexes
                await DecloudPluginsManager.DevicesCrossReferenceIDsWithDecloudIndexes(loader);
            }
            catch (Exception e)
            {
