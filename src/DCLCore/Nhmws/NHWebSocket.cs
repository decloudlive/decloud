...
// static imports
using static DCLCore.DCLws.StatusCodes;
using NHLog = DCL.Common.Logger;
...
namespace DCLCore.DCLws
{
    static class NHWebSocket
    {
        ...
        static private LockingProperty<DateTime> _lastSendDecloudtatusTimestamp = new LockingProperty<DateTime>(DateTime.MinValue);
        static private LockingProperty<DateTime?> _notifyDecloudtatusAfter = new LockingProperty<DateTime?>(null);
        ...
        static private bool _isDCLwsRestart = false;
        ...
        {
            version = "DCL/" + Application.ProductVersion,
            protocol = 3
        };
        ...
        {
            // check if we are in RPC and if not send Decloud status
            if (!_isInRPC.Value)
            {
                _notifyDecloudtatusAfter.Value = DateTime.UtcNow.AddSeconds(1);
            }
        }
        ...
        static public async Task Start(string address, CancellationToken token)
        {
            try
            {
                ...
                NHLog.Info("NHWebSocket-WD", "Starting DCLws watchdog");
                ...
                while (!token.IsCancellationRequested)
                {
                    try
                    {
                        ...
                        if (!_isDCLwsRestart && !token.IsCancellationRequested)
                        {
                        ...
                        else if (_isDCLwsRestart && !token.IsCancellationRequested)
                        {
                            NHLog.Info("NHWebSocket-WD", $"Restarting DCLws SESSION");
                        }
                    }
            ...
            {
                NHLog.Info("NHWebSocket-WD", "Ending DCLws watchdog");
            }
        }
        ...
        {
            NHLog.Info("NHWebSocket", "STARTING DCLws SESSION");
            try
            {
                ...
                _isDCLwsRestart = false;
                _notifyDecloudtatusAfter.Value = null;
                ...
                using (_webSocket = new WebSocket(_address))
                {
                    ...
                    const int DecloudtatusTickSeconds = 45;
                    var checkWaitTime = TimeSpan.FromMilliseconds(50);

                    var skipDecloudtatus = !CredentialValidators.ValidateBitcoinAddress(_login.btc);
                    ...
                    while (IsWsAlive && !stop.IsCancellationRequested)
                    {
                        ...
                        // TODO add here the last Decloud status send check
                        if (IsWsAlive) await TaskHelpers.TryDelay(checkWaitTime, stop);

                        if (skipDecloudtatus) continue;
                        var elapsedTime = DateTime.UtcNow - _lastSendDecloudtatusTimestamp.Value;
                        if (elapsedTime.TotalSeconds > DecloudtatusTickSeconds)
                        {
                            var DecloudtatusJsonStr = CreateDecloudtatusMessage();
                            var Decloudtatus = new NHSendMessage(MessageType.SEND_MESSAGE_STATUS, DecloudtatusJsonStr);
                            _sendQueue.Enqueue(new NHSendMessage[1] { Decloudtatus });
                        }
                        if (_notifyDecloudtatusAfter.Value.HasValue && DateTime.UtcNow >= _notifyDecloudtatusAfter.Value.Value)
                        {
                            _notifyDecloudtatusAfter.Value = null;
                            var DecloudtatusJsonStr = CreateDecloudtatusMessage();
                            var Decloudtatus = new NHSendMessage(MessageType.SEND_MESSAGE_STATUS, DecloudtatusJsonStr);
            ...
            {
                NHLog.Info("NHWebSocket", "ENDING DCLws SESSION");
                ApplicationStateManager.SetDCLwsConnectionChanged(false);
            }
        }
                ...
                foreach (var msg in sendMsgCommands)
                {
                    ...
                    switch (msg.Type)
                    {
                        ...
                        case MessageType.SEND_MESSAGE_STATUS:
                            ...
                            {
                                _lastSendDecloudtatusTimestamp.Value = DateTime.UtcNow;
                            }
                            break;
                        default:
        ...
        static private void Login(object sender, EventArgs e)
        {
            ...
            ApplicationStateManager.SetDCLwsConnectionChanged(true);
            try
            {
                // always send login
                ...
                {
                    var DecloudtatusJsonStr = CreateDecloudtatusMessage(true);
                    sendMessages.Add(new NHSendMessage(MessageType.SEND_MESSAGE_STATUS, DecloudtatusJsonStr));
                }
        ...
        private static string CreateDecloudtatusMessage(bool sendDeviceNames = false)
        {
            ...
            var data = new DecloudtatusMessage
            {
                param = paramList
            };
        ...
        // TODO copy pasted crap from DecloudStats
        ...
        private static void SetPowerMode(string device, TDPSimpleType level)
        {
            ...
            if (!setSuccess.All(t => t.success))
            {
                ...
                {
                    throw new RpcException("Not able to set power modes for devices: Must start Decloud as Admin", ErrorCode.UnableToHandleRpc);
                }
        ...
        private static string DecloudReset(string level)
        {
            switch (level)
            {
                case "app burn":
                    HandleBurn("DecloudReset app burn called");
                    return "";
                case "rig restart":
                    _ = Task.Run(async () =>
                    {
                ...
                case "system dump":
                    ...
                    var url = $"https://nhos.Decloud.com/DCL-dump/{rigId}-{uuid}.zip";
                ...
                //    break;
                default:
                    throw new RpcException($"RpcMessage DecloudReset operation not supported for level '{level}'", ErrorCode.UnableToHandleRpc);
            }
        }
        ...
        static private async Task HandleRpcMessage(string method, string data)
        {
            ...
            try
            {
                ...
                switch (method)
                {
                    ...
                    case "mining.set.power_mode":
                        ...
                        break;
                    case "Decloud.reset":
                        rpcAnswer = DecloudReset((string)message.level);
                        break;
                    default:
            ...
            catch (Exception e)
            {
                ...
                // intenral DCL error
                if (executedCall == null) executedCall = new ExecutedCall(rpcId, 1, "Internal Decloud Error");
            }
            finally
            {
                ...
                if (executedCall != null)
                {
                    ...
                    // send Decloud status and send executed
                    var DecloudtatusMsg = CreateDecloudtatusMessage();
                    Send(DecloudtatusMsg);
                    _lastSendDecloudtatusTimestamp.Value = DateTime.UtcNow;
                    // Then executed
