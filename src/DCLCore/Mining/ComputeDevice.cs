...
namespace DCLCore.Mining
{
    public class ComputeDevice : NotifyChangedBase
    {
        ...
        public void RemovePluginAlgorithms(string pluginUUID)
        {
            ...
            var toRemove = AlgorithmSettings.Where(algo => algo.Algorithm.DecloudID == pluginUUID);
        ...
        public AlgorithmContainer GetAlgorithm(string DecloudUUID, params AlgorithmType[] ids)
        {
            return AlgorithmSettings.FirstOrDefault(a => a.DecloudUUID == DecloudUUID && !a.IDs.Except(ids).Any());
        }
        ...
        public void SetDeviceConfig(DeviceConfig config)
        {
            ...
            foreach (var pluginConf in config.PluginAlgorithmSettings)
            {
                ...
                var pluginAlgo = AlgorithmSettings
                    .Where(pAlgo => pluginConf.PluginUUID == pAlgo.Algorithm.DecloudID && pluginConfAlgorithmIDs.Except(pAlgo.Algorithm.IDs).Count() == 0)
                    .FirstOrDefault();
        ...
        public DeviceConfig GetDeviceConfig()
        {
            ...
            foreach (var algo in AlgorithmSettings)
            {
                var pluginConf = new PluginAlgorithmConfig
                {
                    ...
                    PluginUUID = algo.Algorithm.DecloudID,
