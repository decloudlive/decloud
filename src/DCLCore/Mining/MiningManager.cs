...
namespace DCLCore.Mining
{
    public static class MiningManager
    {
        ...
        // TODO make sure _miningDevices and _runningDecloud are in sync
        ...
        private static Dictionary<string, Decloud> _runningDecloud = new Dictionary<string, Decloud>();
        ...
        private class DecloudRestartLoopNotifyCommand : MainCommand
        { }
        ...
        public static Task DecloudRestartLoopNotify()
        {
            if (RunninLoops == null) return Task.CompletedTask;
            var command = new DecloudRestartLoopNotifyCommand();
            _commandQueue.Enqueue(command);
            return command.Tsc.Task;
        }
        ...
        private static async Task HandleCommand(MainCommand command)
        {
            ...
            try
            {
                await CheckGroupingAndUpdateDecloud(command);
            }
            catch (Exception e)
            {
        ...
        private static async Task StopAndRemoveDecloud(DeferredDeviceCommand c)
        {
            var stopMiningKey = _runningDecloud.Keys.ToArray().Where(key => key.Contains(c.device.Uuid)).FirstOrDefault();
            if (stopMiningKey != null)
            {
                await _runningDecloud[stopMiningKey].StopTask();
                _runningDecloud.Remove(stopMiningKey);
            }
        }
        ...
        private static async Task HandleDeferredCommands(List<DeferredDeviceCommand> deferredCommands)
        {
            try
            {
                ...
                // stop all newly obsolete Decloud 
                foreach (var stopDecloud in nonMiningCommands) await StopAndRemoveDecloud(stopDecloud);
                ...
                var devicesToMine = _miningDevices
                    ...
                    .ToArray();
                var anyMiningDeviceInactive = devicesToMine.Any(dev => !_runningDecloud.Keys.Any(key => key.Contains(dev.Uuid)));
                ...
                if (devicesToMineChange || anyMiningDeviceInactive || miningDevicesMissmatch)
                {
                    ...
                    //    }
                    //}
                    await CheckGroupingAndUpdateDecloud(new MainCommand());
                }
        ...
        private static async Task MiningManagerCommandQueueLoop(CancellationToken stop)
        {
            ...
            finally
            {
                ...
                foreach (var groupDecloud in _runningDecloud.Values)
                {
                    await groupDecloud.StopTask();
                }
                _runningDecloud.Clear();
        ...
        private static async Task StopAllDecloudTask()
        {
            foreach (var groupDecloud in _runningDecloud.Values)
            {
                await groupDecloud.StopTask();
        ...
        // PauseAllDecloud doesn't clear _miningDevices
        private static async Task PauseAllDecloud()
        {
            foreach (var groupDecloud in _runningDecloud.Values)
            {
                await groupDecloud.StopTask();
            ...
            //foreach (var groupDecloud in _runningDecloud.Values)
            //{
            //    foreach (var pair in groupDecloud.MiningPairs)
            //    {
        ...
        //private static async Task ResumeAllDecloud()
        //{
        //    foreach (var groupDecloud in _runningDecloud.Values)
        //    {
        //        await groupDecloud.StartDecloudTask(stopMiningManager, _miningLocation, _username);
        //    }
        //    // TODO resume devices to Mining state
        //}

        private static async Task CheckGroupingAndUpdateDecloud(MainCommand command)
        {
            // #1 parse the command
            ...
            {
                // TODO this is Decloud restarts on mining location and username change, you should take into account the mining location changed for benchmarking
                // RESTART-STOP-START
                // STOP
                foreach (var Decloud in _runningDecloud.Values) await Decloud.StopTask();
                // START
                foreach (var Decloud in _runningDecloud.Values) await Decloud.StartDecloudTask(stopMiningManager, _miningLocation, _username);
            }
            else if (_miningDevices.Count == 0)
            {
                await StopAllDecloudTask();
                ApplicationStateManager.StopMining();
            }
            else
            {
                ApplicationStateManager.StartMining();
                bool skipProfitsThreshold = command is MiningProfitSettingsChangedCommand || command is DecloudRestartLoopNotifyCommand;
        ...
        {
            if (MiningProfitSettings.Instance.DecloudegardlessOfProfit)
            {
        ...
        private static async Task SwichMostProfitableGroupUpMethodTask(Dictionary<AlgorithmType, double> normalizedProfits, bool skipProfitsThreshold)
        {
            ...
            var currentRunningGroups = _runningDecloud.Keys.ToArray();
            // check which groupDecloud should be stopped and which ones should be started and which to keep running
            var noChangeGroupDecloudKeys = newGroupedMiningPairs.Where(pair => currentRunningGroups.Contains(pair.Key)).Select(pair => pair.Key).OrderBy(uuid => uuid).ToArray();
            var toStartDecloudGroupKeys = newGroupedMiningPairs.Where(pair => !currentRunningGroups.Contains(pair.Key)).Select(pair => pair.Key).OrderBy(uuid => uuid).ToArray();
            var toStopDecloudGroupKeys = currentRunningGroups.Where(runningKey => !newGroupedMiningPairs.Keys.Contains(runningKey)).OrderBy(uuid => uuid).ToArray();
            // first stop currently running
            foreach (var stopKey in toStopDecloudGroupKeys)
            {
                var stopGroup = _runningDecloud[stopKey];
                _runningDecloud.Remove(stopKey);
                await stopGroup.StopTask();
            }
            // start new
            foreach (var startKey in toStartDecloudGroupKeys)
            {
                var miningPairs = newGroupedMiningPairs[startKey];
                var toStart = Decloud.CreateDecloudForMining(miningPairs, startKey);
                if (toStart == null)
                {
                    Logger.Error(Tag, $"CreateDecloudForMining for key='{startKey}' returned <null>");
                    continue;
                }
                _runningDecloud[startKey] = toStart;
                await toStart.StartDecloudTask(stopMiningManager, _miningLocation, _username);
            }
            // log scope
            {
                var stopLog = toStopDecloudGroupKeys.Length > 0 ? string.Join(",", toStopDecloudGroupKeys) : "EMTPY";
                Logger.Info(Tag, $"Stop Old Mining: ({stopLog})");
                var startLog = toStartDecloudGroupKeys.Length > 0 ? string.Join(",", toStartDecloudGroupKeys) : "EMTPY";
                Logger.Info(Tag, $"Start New Mining : ({startLog})");
                var sameLog = noChangeGroupDecloudKeys.Length > 0 ? string.Join(",", noChangeGroupDecloudKeys) : "EMTPY";
                Logger.Info(Tag, $"No change : ({sameLog})");
            }
        }

        // TODO check the stats calculation
        //public static async Task DecloudtatsCheck()
        //{
        //    await _semaphore.WaitAsync();
        //    try
        //    {
        //        foreach (var m in _runningDecloud.Values)
        //        {
