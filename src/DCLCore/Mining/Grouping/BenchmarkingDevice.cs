...
using DCLCore.Mining.Benchmarking;
...
namespace DCLCore.Mining.Grouping
{
        ...
        private async Task<object> BenchmarkAlgorithm(AlgorithmContainer algo, CancellationToken stop)
        {
            ...
            try
            {
                ...
                using (var powerHelper = new PowerHelper(algo.ComputeDevice))
                {
                    ...
                    var Decloud = plugin.CreateDecloud();

                    // check ethlargement
                    EthlargementIntegratedPlugin.Instance.Start(miningPairs);
                    Decloud.InitMiningPairs(miningPairs);
                    // fill service since the benchmark might be online. DemoUser.BTC must be used
                    Decloud.InitMiningLocationAndUsername(miningLocation, DemoUser.BTC);
                    powerHelper.Start();
                    algo.ComputeDevice.State = DeviceState.Benchmarking;
                    var result = await Decloud.StartBenchmark(stop, PerformanceType);
            ...
            try
            {
                ...
                while (benchAlgos.Any() && !_stopBenchmark.IsCancellationRequested)
                {
                    ...
                    try
                    {
                        ...
                        await Task.Delay(MiningSettings.Instance.DecloudRestartDelayMS, _stopCurrentAlgorithmBenchmark.Token);
                    }
