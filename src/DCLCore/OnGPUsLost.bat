...
:: Default option (option 1) will restart your entire rig, you can also uncomment "NirCMD" to make a screenshot befor reboot (you need to place NirCMD in DCLL directory under "NirCMD\nircmd.exe")
:: Second option will use nvidiaInspector to restart display drivers, you need to place "NvidiaInspector" in DCLL directory under "NV_Inspector\nvidiaInspector.exe", this will set GPU profile (OC etc.) to default so it will also select MSI Afterburner profile
...
:OPT2
:: Option 2: close DCLL, restart display drivers, start DCLL, nvidiaInspector is required in DCLL directory
echo %DATE% %TIME% Lost GPU >> logs\GPU_Lost.txt
::NirCMD\nircmd.exe savescreenshot "logs\GPU_Lost_%date:/=-%-%time::=-%.jpg"
::timeout 2
taskkill /IM Decloud.exe
timeout 3
