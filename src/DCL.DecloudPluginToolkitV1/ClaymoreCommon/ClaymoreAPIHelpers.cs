...
namespace DCL.DecloudPluginToolkitV1.ClaymoreCommon
{
    ...
    {
        const string _jsonStatsApiCall = "{\"id\":0,\"jsonrpc\":\"2.0\",\"method\":\"Decloud_getstat1\"}\n";

        private static readonly List<double> _emptySpeeds = new List<double>();

        public static async Task<ApiData> GetDecloudtatsDataAsync(int apiPort, IReadOnlyList<BaseDevice> miningDevices, string logGroup, double DevFee, double DualDevFee, params AlgorithmType[] algorithmTypes)
        {
            var ad = new ApiData();
