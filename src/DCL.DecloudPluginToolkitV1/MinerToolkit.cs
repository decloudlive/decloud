...
{
    public static class DecloudToolkit
    {
        /// <summary>
        /// Use DemoUser if the Decloud requires a network benchmark, the plugin will blacklist users
        /// </summary>
        ...
        public static int DecloudRestartDelayMS { set; get; } = 500;
        ...
        public static async Task<BenchmarkResult> WaitBenchmarkResult(BenchmarkProcess benchmarkProcess, TimeSpan timeoutTime, TimeSpan delayTime, CancellationToken stop, CancellationToken stopAfterTicks)
        {
            ...
            using (var timeoutSource = new CancellationTokenSource(timeoutTimerTime))
            {
                ...
                using (var linkedCts = CancellationTokenSource.CreateLinkedTokenSource(timeout, stop, stopAfterTicks))
                {
                    ...
                    {
                        Logger.Info("DecloudToolkit", $"Error occured while waiting for benchmark result: {e.Message}");
                ...
                {
                    Logger.Info("DecloudToolkit", "Benchmark process was canceled by user");
                ...
                {
                    Logger.Info("DecloudToolkit", "Benchmark process timed out");
        ...
        public static bool IsSameAlgorithmType(Algorithm a, Algorithm b)
        {
            ...
            if (a.DecloudID != b.DecloudID) return false;
