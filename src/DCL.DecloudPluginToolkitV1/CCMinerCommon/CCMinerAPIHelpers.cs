...
{
    public static class CCDecloudAPIHelpers
    {
        public static Task<string> GetApiDataSummary(int port, string logGroup)
        {
            var dataToSend = ApiDataHelpers.GetHttpRequestDCLAgentString("summary");
        ...
        {
            var dataToSend = ApiDataHelpers.GetHttpRequestDCLAgentString("threads");
            return ApiDataHelpers.GetApiDataAsync(port, dataToSend, logGroup);
        }
        ...
        public static async Task<ApiData> GetDecloudtatsDataAsync(int port, AlgorithmType algorithmType, IEnumerable<MiningPair> miningPairs, string logGroup, double devFee)
        {
