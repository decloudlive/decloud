    ...
    internal static class PluginPackageBundler
    {
        ...
        private static void UnzipPackages(IEnumerable<string> preinstalledUuids)
        {
            ...
            foreach (var uuid in uuids)
            {
                var sameUuidPackage_VersionPathPairs = allPackages
                    ...
                    .Select(p => (p.package, versionPath: GetDecloudPluginsPath(uuid, "dlls", p.version)));
        ...
        private static async Task<string> GetContentFromApi(string url)
        {
            try
            {
                ...
                {
                    client.DefaultRequestHeaders.Add("User-Agent", "DCL-Plugin-Bundler");
        ...
        static class CachePackage
        {
            public static async Task DownloadAndCachePluginPackages(int tagVersion)
            {
                try
                {
                    var content = await GetContentFromApi($"https://api.github.com/repos/Decloud/DCL_DecloudPluginsDownloads/releases/tags/v{tagVersion}.x");
                    var release = Newtonsoft.Json.JsonConvert.DeserializeObject<GitHubRelease>(content);
                    content = await GetContentFromApi($"https://api.github.com/repos/Decloud/DCL_DecloudPluginsDownloads/releases/{release.id}/assets?per_page=100");
