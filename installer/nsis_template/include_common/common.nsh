...
# this will delete all but configs, backups, internals and Decloud_plugins
!macro deleteUserData_KeepNonGeneralSettings
  ...
  Delete "$INSTDIR\Decloud.exe"
  Delete "$INSTDIR\Decloud.exe.config"
  Delete "$INSTDIR\Decloud.pdb"
  Delete "$INSTDIR\OnGPUsLost.bat"
  Delete "$INSTDIR\runDCLasadmin.exe"
  Delete "$INSTDIR\runDCLasadmin.exe.config"
  Delete "$INSTDIR\Uninstall Decloud.exe"
  # Because you f-ed up
  ...
  DeleteRegKey HKCU "SOFTWARE\Decloud"
!macroend
